...
function usage() {
  console.error('Usage: node scripts/validate-vsqx-semantics.mjs [fixturesRoot] [defaultLyric]');
}
...
function diffSection(label, expected, actual) {
  const e = toStableJson(expected);
  const a = toStableJson(actual);
  if (e === a) return null;
...
async function validateOne(inputPath, defaultLyric) {
  ...
  if (sourceProject.tracks.length !== parsedProject.tracks.length) {
    failures.push(
      `Track count mismatch: ${sourceProject.tracks.length} != ${parsedProject.tracks.length}`,
    );
  }
  ...
  const tempoDiff = diffSection(
    'tempos',
    normalizeTempos(sourceProject),
    normalizeTempos(parsedProject),
  );
