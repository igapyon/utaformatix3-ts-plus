...
function usage() {
  console.error("Usage: node scripts/validate-vsqx-roundtrip-diff.mjs [fixturesRoot] [defaultLyric] [mode]");
...
try {
  ...
  for (const inputPath of inputs) {
    ...
    const musicXml = convertVsqxToMusicXml(inputText, { defaultLyric });
    const roundtripVsqx = convertMusicXmlToVsqx(musicXml, { musicXml: { defaultLyric } });
    const roundtripProject = parseVsqx(roundtripVsqx, { defaultLyric });

    const failures = collectDiffs(sourceProject, roundtripProject, defaultLyric);
    const textDiff = firstDiffIndex(inputText, roundtripVsqx);
